jQuery(document).ready(function($){
    ...
    $("input#inputStartDate, input#inputEndDate").each(function(){
        $(this).datetimepicker({
            language:  'zh-CN',
            format: $("input", $(this)).attr("data-date-format") || "yyyy-mm-dd hh:ii",
            autoclose: true,
    ...
    function delimage($this) {
        ...
        var $that = $this;
        $.post($base + "/agency/" + $control + "/delimage/" + $id, {name: $name}, function(out){
            if (out.status == 0) {
    ...
    $("#myModal.upload").on("show", function(e) {
        var $id = $("input[type='hidden'][name='id']:first").attr("value");
        if (!$id) {
            alert("请先填写车辆基本信息并保存，再上传图片！");
            e.preventDefault();
        } else {
            var $base = $("input[type='hidden'][name='base']:first").attr("value");
            var $control = $("input[type='hidden'][name='control']:first").attr("value");
            ...
            $('#upload').fileupload({
                url : $base + "/agency/" + $control + "/upload/" + $id,
                dataType: 'json',
                autoUpload: true,
    ...
    // Promotion
    // ----------------------------------------------------
    var $base = $("#promotion input[name='base'][type='hidden']").attr("value");
    ...
    $("#promotion form").submit(function(){
        ...
        // Submit
        var id = $("input[type='hidden'][name='id']").val();
        var $base_promotion = $("input[type='hidden'][name='base-promotion']:first").attr("value");
        $.post($(this).attr("action"), $(this).serialize(), function(data){
            $msg = $("#msg");
            if (data.status == 0) {
                $msg.attr("class","alert fade in");
                ...
                $msg.alert();
                if (!id) {
                    setTimeout(function(){location.href = $base_promotion + "/agency/promotion/edit/" + data.id;}, 1000);
                }
            } else {
                $msg.attr("class","alert alert-block alert-error fade in");
                $msg.text(data.msg);
    ...
    $("#promotion #delete-modal button[name='delete']").click(function(){
        $("button").attr("disabled","true");
        var $base = $("input[type='hidden'][name='base-promotion']:first").attr("value");
        var id = $("input[type='hidden'][name='id']").val();
        $.post($base + "/agency/promotion/delete", {id: id}, function(data){
            if (data.status == 0) {
                $("#delete-modal-msg").attr("class","alert fade in");
                $("#delete-modal-msg").text(data.msg);
                $("#delete-modal-msg").alert();
                setTimeout(function(){location.href = $base + "/agency/promotion/";}, 1000);
            } else {
                $("#delete-modal-msg").attr("class","alert alert-block alert-error fade in");
                $("#delete-modal-msg").text(data.msg);
                $("#delete-modal-msg").alert();
        ...
        $.post(post_url, $(this).serialize(), function(data){
            if (data.status == 0) {
                if (!isNaN(parseInt(data.id))) {
                    $("input[type='hidden'][name='id']:first").val(data.id);
                    $("#car form").attr("action", $base + "/agency/agencycar/save/" + data.id);
                }
                $("#msg").attr("class","alert fade in");
                $("#msg").text(data.msg);
                $("#msg").alert();
    ...
    $('#car-list button.delete').click(function(){
        if (confirm('确定删除吗?')) {
            ...
            var id = $(this).attr('data-id');
            var $that = $(this);
            $.post($base + '/agency/agencycar/del/' + id, function(data){
                if (data.status == 0) {
                    $("#msg").attr("class","label alert fade in");
                    $("#msg").text(data.msg);
                    $("#msg").alert();
    ...
    // carousel
    $('#carousels a').click(function() {
        $("input[type='hidden'][name='id']:first").attr("value", $(this).attr("value"));
        return true;
    });

    $("#carouselModal.upload").on("show", function() {
        var $id = $("input[type='hidden'][name='id']:first").attr("value");
        var $base = $("input[type='hidden'][name='base']:first").attr("value");
        var $control = $("input[type='hidden'][name='control']:first").attr("value");
        var $thumb = $("input[type='hidden'][name='base-thumb']:first").attr("value");
        ...
        $('#upload').fileupload({
            ...
            progressall: function (e, data) {
                ...
                if (data.textStatus != "success" || data.result.status != 0){
                    ...
                    data.result.msg && $label.text(data.result.msg).addClass("label-success").removeClass("label-important");
                    setTimeout(function(){
                        $("#carouselModal.upload").modal("hide");
                        location.reload();},
                        1000);
                }
            }
