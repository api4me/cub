jQuery(document).ready(function($){
    ...
    $("#purchase-form").submit(function(){
        ...
        $.post($(this).attr("action"), $(this).serialize(), function(data){
            if (data.status == 0) {
                $("#purchase_msg").attr("class","alert fade in");
                $("#purchase_msg").text(data.msg);
                $("#purchase_msg").alert();
                setTimeout(function(){
                    $that.get(0).reset();
                    $("#purchase_msg").removeClass("alert in").text("");
                }, 3000);
            } else {
                $("#purchase_msg").attr("class","alert alert-block alert-error fade in");
                $("#purchase_msg").text(data.msg);
                $("#purchase_msg").alert();
                $("button").removeAttr("disabled");
            }
    ...
    $("#buy-car #price").focus(function(){
        ...
        $.post(url + "/buy/pay", {id: id, p: p, v: v}, function(data){
            if (data.status != 0) {
                ...
                var limit_num = $('#buy-car span.limit').attr('data-limit');
                if (limit_num > 1) {
                    $('#buy-car span.limit').text('剩余竞拍' + (limit_num - 1) + '次');
                    $('#buy-car span.limit').attr('data-limit', limit_num - 1)
                } else {
                    $('#buy-car span.limit').text('竞拍数已完');
                }
            }
            $("button").removeAttr("disabled");
        });
