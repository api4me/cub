...
/*{{{ upload */
    ...
    public function upload4agency($id) {
        $out = array();
        // Fix IE issue, fileupload jquery plug is not send "HTTP_X_REQUESTED_WITH"
        /*
        if (!$this->CI->input->is_ajax_request()) {
        ...
        */
        ...
        // Update in DB
        $this->CI->load->model("magencycar");
        $car = $this->CI->magencycar->load($id);
        $images = json_decode($car->images, true);
        if (!$images) {
            $images = array();
        }
        $image_name = $upload["raw_name"] . "_i" . $upload["file_ext"];
        $images[] = $image_name;
        $this->CI->magencycar->save(array("images"=>json_encode($images)), $id);
...
/*}}}*/
    ...
    public function delimage4agency($id){
        $out = array();
        if (!$this->CI->input->is_ajax_request()) {
            $out["status"] = 1;
            $out["msg"] = "系统忙，请稍后...";
